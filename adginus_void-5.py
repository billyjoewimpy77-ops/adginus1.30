...
class PatternRecognitionEngine:
    def __init__(self):
        ...
        self.known_patterns = {}
        self.anomalies = []
        self.correlations = {}  # pair_key (sorted tuple) -> correlation dict; was defaultdict(list)
    ...
    def _find_correlations(self, elements):
        # FIX: this used to do self.correlations[elem].append(c) — with
        # correlations as defaultdict(list), every single observe() call
        # where the same two elements co-occurred again appended ANOTHER
        # near-duplicate entry, forever, for the entire lifetime of the
        # process. Any two words that show up together regularly (which
        # is exactly the case this is meant to detect) would grow that
        # list without bound. Now keyed by the sorted pair itself, so
        # each pair has exactly one entry that gets updated in place.
        corrs = []
        for elem in elements:
            ...
            for other, count in top_co:
                if count >= 3:
                    ...
                    if strength > 0.3:
                        ...
                        pair_key = tuple(sorted([elem, other]))
                        self.correlations[pair_key] = c
                        corrs.append(c)
        return corrs[:5]
...
class VoidPersistence:
        ...
        try:
            ...
            state = {
                ...
                "memory": {
                    ...
                    # FIX: build a copy with embeddings stripped for the
                    # main file (embeddings go to a companion file below).
                    # The pasted version of this used node.pop("embedding")
                    # directly on memory.long_term — that MUTATES the live,
                    # in-memory data, not just what gets serialized, which
                    # would silently break semantic recall the moment you
                    # called save() until the next load(). This builds a
                    # shallow copy per node instead, so live memory is
                    # never touched.
                    "long_term": {
                        h: ({k: v for k, v in node.items() if k != "embedding"} if "embedding" in node else node)
                        for h, node in memory.long_term.items()
                    },
            ...
            # Companion embeddings file, written separately (and atomically,
            # same pattern as above) so the main save stays human-readable
            # and doesn't balloon with vector data.
            # FIX: the pasted version derived this with
            # filepath.replace(".json", "_embeddings.json") — if filepath
            # doesn't end in .json, that .replace() is a silent no-op and
            # emb_path becomes IDENTICAL to filepath, so the embeddings
            # write would clobber the main save file. os.path.splitext
            # handles this correctly regardless of extension.
            embeddings_store = {
                h: node["embedding"] for h, node in memory.long_term.items() if node.get("embedding")
            }
            if embeddings_store:
                base, ext = os.path.splitext(filepath)
                emb_path = f"{base}_embeddings{ext or '.json'}"
                emb_fd, emb_temp_path = tempfile.mkstemp(dir=dir_name, suffix=".tmp")
                try:
                    with os.fdopen(emb_fd, 'w', encoding='utf-8') as tmp:
                        json.dump(embeddings_store, tmp)
                    os.replace(emb_temp_path, emb_path)
                finally:
                    if os.path.exists(emb_temp_path):
                        try:
                            os.remove(emb_temp_path)
                        except:
                            pass

            return {"status": "SAVED", "filepath": filepath, "saved_at": timestamp, "memories": len(memory.long_term), "knowledge": len(learner.knowledge), "embeddings_saved": len(embeddings_store)}
    ...
    def load_state(void_instance, filepath: str = None) -> Dict:
        ...
        try:
            ...
            # Reattach embeddings from the companion file, if present.
            # Same safe filename derivation as save_state — os.path.splitext
            # instead of a naive .replace(".json", ...).
            base, ext = os.path.splitext(filepath)
            emb_path = f"{base}_embeddings{ext or '.json'}"
            if os.path.exists(emb_path):
                try:
                    ...
                    for h, vec in embeddings_store.items():
                        if h in memory.long_term:
                            memory.long_term[h]["embedding"] = vec
                except Exception:
                    pass  # missing/corrupt embeddings file shouldn't fail the whole load
...
class BackgroundCycle:
    ...
    def _do_think(self):
        ...
        try:
            # Opportunistic embedding backfill: if a Voyage key/proxy was
            # added after some memories were already stored (or the
            # embedder was briefly unreachable when they were stored),
            # this catches them up gradually in the background instead of
            # requiring a manual backfill_embeddings() call. Small batch
            # size keeps this cheap per cycle.
            if self.void.memory.embedder and self.void.memory.embedder.is_available():
                backfill = self.void.memory.backfill_embeddings(batch_size=20)
                if backfill.get("backfilled", 0) > 0:
                    think_report["backfill"] = backfill
...
class TheVoid:
    ...
    def dream(self, duration: int = 50) -> Dict:
        with self._lock:
            # FIX: was a flat -0.05 regardless of duration, so a 10-cycle
            # dream cost the same energy as a 500-cycle dream. Now scales
            # with duration instead (e.g. -0.01 at duration=50, -0.1 at
            # duration=500) so longer dreams actually cost more energy.
            self.energy = max(0.0, self.energy - 0.01 * (duration / 50.0))
